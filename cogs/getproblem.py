...
class numcog(commands.Cog, name="numCommanding"):
    ...
    async def 백준유저검색(self, ctx, *informations):
        ...
        for info in informations:
            ...
            percentage = boj_calcul100(user_info)
            em = discord.Embed(title=info, color=boj_colorselect(user_info["level"]), url="https://solved.ac/profile/"
                                                                                          + info)
            em.set_thumbnail(url=boj_levels[user_info["level"]])
            em.add_field(name="solved", value=user_info["solved"], inline=1)
            em.add_field(name="exp", value= f"{percentage[2]}%  [{percentage[0]}/{percentage[1]}]", inline=1)
            await ctx.send(embed=em)
